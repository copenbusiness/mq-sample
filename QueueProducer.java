...
public class QueueProducer {
    ...
    public static void main(String[] args) {
        if (args.length > 1) {
            System.err.println(
                    "Program takes at most one argument: [<number-of-messages>]");
            System.exit(1);
        }
        ...
        if (args.length == 1) {
            try {
                ...
                if (numMsgs <= 0) {
                    System.err.println("Number of messages must be positive: " + numMsgs);
                    System.exit(1);
                }
            } catch (NumberFormatException e) {
                System.err.println("Argument must be a valid number: " + args[0]);
                System.exit(1);
            }
        }
